...
/// contract and to shape the "browse key" text the host generates when a key node is double-clicked. That
/// generated text (<c>SELECT * FROM "key" LIMIT n OFFSET m</c>) is parsed back by
/// <see cref="DragonflyDbProvider"/> (<see cref="DragonflyDbQuery"/>) into a live <c>TYPE</c> lookup
/// followed by the matching typed command (<c>GET</c>/<c>HGETALL</c>/<c>LRANGE</c>/<c>SMEMBERS</c>/
/// <c>ZRANGE</c>) — the same "blessed pseudo-SQL, recognised only by the provider's own parser" convention
/// as the Redis and MongoDB dialects.
/// </summary>
public sealed class DragonflyDbDialect : ISqlDialect
{
    public IReadOnlySet<string> Keywords { get; } = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

    // Key names can contain almost anything; double-quote for the generated browse text, same as Redis.
    // DragonflyDbQuery strips the quotes again before treating it as a key name.
