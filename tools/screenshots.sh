...
# scene renders the actual views/view-models via DataTray.Screenshots against a throwaway config dir
# and a synthetic demo database — see src/DataTray.Screenshots. Run after a UI change that the README
# should reflect; commit the updated PNGs under docs/images/.
set -euo pipefail
cd "$(dirname "$0")/.."

run() { dotnet run --project src/DataTray.Screenshots -c Debug -- "$@"; }
