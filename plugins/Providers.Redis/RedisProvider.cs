...
/// host's (SQL-shaped) <see cref="IDbProvider"/> contract honestly, following the MongoDB provider's
/// pattern (SE-114):
/// <list type="bullet">
/// <item>Schema tree: connection root → DB indices (Redis' numbered logical databases) → keys, grouped one
///   level deep by a <c>:</c>-prefix (the common Redis naming convention).</item>
/// <item>Queries: the editor holds typed command lines (<c>GET key</c> / <c>HGETALL key</c> / …), parsed
///   by <see cref="RedisQuery"/> and run through the native driver.</item>
/// <item>Editable grid: only for Hash keys, only edit/delete an existing field's value (not add a new
///   field — no cell to type its name into once "field" is IsReadOnly, see <see cref="ApplyChangesAsync"/>).
///   String/List/Set/ZSet keys stay read-only in the grid; mutate them via console commands
///   (<c>SET</c>/<c>LPUSH</c>/<c>SADD</c>/<c>ZADD</c>/…) — a deliberate MVP scope cut, not an oversight
///   (see the plugin's README).</item>
...
/// </summary>
public sealed class RedisProvider : IDbProvider
{
    ...
    private async Task<IReadOnlyList<DbTreeNode>> LoadDatabasesAsync(IConnectionMultiplexer multiplexer, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (config.Length > 0 && int.TryParse(config[0].Value, out var configured))
            {
                databaseCount = configured;
            }
        }
        catch
        {
            // CONFIG GET is disabled on some managed/ACL-restricted deployments — fall back to the default.
        }
    ...
    public async Task<QueryResult> ExecuteQueryAsync(ConnectionProfile profile, string sql, CancellationToken ct)
    {
        ...
        var query = RedisQuery.Parse(sql);
        await using var multiplexer = await ConnectAsync(profile);
        var db = GetDatabase(multiplexer, profile);

        return query.Kind == RedisQueryKind.Browse
            ? await BrowseKeyAsync(db, query, stopwatch.Elapsed)
            : await RunCommandAsync(db, query.Command, query.Args, stopwatch.Elapsed);
    }

    // Redis has no multi-statement scripts in the SQL sense; a "script" is one command line per line.
    ...
    // Redis has no query planner (direct key access is O(1)/O(log N) depending on the command) — surface
    // that fact plus the key's TYPE/encoding as the closest useful equivalent, same spirit as Mongo
    // returning its plan document as a single JSON-ish cell.
    public async Task<QueryResult> ExplainAsync(ConnectionProfile profile, string sql, CancellationToken ct)
    {
        var stopwatch = Stopwatch.StartNew();
        var query = RedisQuery.Parse(sql);
        var key = query.Kind == RedisQueryKind.Browse ? query.Key : query.Args.FirstOrDefault();
        ...
        if (key is null)
        {
            plan = "Redis has no query planner — every command is a direct, O(1)/O(log N) key operation.";
        }
        else
        {
            ...
            {
                // OBJECT ENCODING may be restricted; the type alone is still useful.
            }

            plan = type == RedisType.None
                ? $"Key \"{key}\" does not exist."
                : $"Key \"{key}\": type={type}{(encoding is null ? "" : $", encoding={encoding}")}. " +
                  "Redis has no query planner — every command is a direct, O(1)/O(log N) key operation.";
        }
    ...
    // sees (and can edit) real Redis syntax in the query tab instead of the browse pseudo-SQL. Needs a live
    // TYPE lookup (StackExchange.Redis exposes true synchronous methods, not just Task-wrapped ones) since
    // a key's shape isn't known until queried — see the design note on IDbProvider.BuildNodeQuery.
    ...
    // "Drop" and "Truncate" on a key: the host previews this text and runs it via ExecuteDdlAsync. Redis
    // has no notion of "clear a key but keep it present" distinct from "the key doesn't exist" — deleting
    // every element of a collection auto-deletes the key — so unlike Mongo's drop()/deleteMany({})
    // (a real distinction for a collection), Drop and Truncate are deliberately the same operation here.
    ...
    public async Task ExecuteDdlAsync(ConnectionProfile profile, string sql, CancellationToken ct)
    {
        ...
        if (query.Kind == RedisQueryKind.Browse)
        {
    ...
    // letting an existing row's field name be edited in place would silently no-op (it's excluded from
    // Modified diffs as a key column either way) rather than rename the hash field. Consequence: "Add
    // Row" cannot supply a field name and is therefore unsupported for Hash keys — reported as a row
    // error, not silently dropped. Add a field via the console instead (HSET key field value).
    public async Task<WritebackResult> ApplyChangesAsync(ConnectionProfile profile, ChangeSet changes, CancellationToken ct)
    {
        ...
        // One MULTI/EXEC transaction for the whole grid save: unlike Mongo's sequential ops, Redis
        // transactions are cheap and don't need a replica set, so this genuinely is atomic.
    ...
    private static IDatabase GetDatabase(IConnectionMultiplexer multiplexer, ConnectionProfile profile)
    {
        ...
        // DefaultDatabase, same convention as Mongo's ResolveDatabase.
    ...
    private async Task<QueryResult> BrowseKeyAsync(IDatabase db, RedisQuery query, TimeSpan elapsed)
    {
        var key = query.Key;
    ...
    // Redis sets are unordered with no native offset/limit paging (SMEMBERS reads the whole set); browse
    // ignores paging here, a known MVP limitation for very large sets (documented in the README).
